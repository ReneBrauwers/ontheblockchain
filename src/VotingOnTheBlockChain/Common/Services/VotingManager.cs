...
namespace Common.Services
{
    public sealed class VotingManager
    {
        ...
        //     new string  ("wss://s1.ripple.com"),
        //     new string ("wss://s2.ripple.com/"),
        //     new string ("wss://x1.sologenic.org")
        //     };
        ...
        public async IAsyncEnumerable<AccountBalance> GetVoterBalancesAsyncEnumerable(List<string> Addresses, string PeerAccount, uint LedgerGetBalanceIndex, string Currency, string socketEndpoint = "wss://xrplcluster.com")
        {
            ...
            CancellationTokenSource cTokenSource = new CancellationTokenSource(new TimeSpan(1, 0, 0)); //allow 1 hour
            ...
            while (currentPage < totalPages)
            {
                ...
                using (var clientWebsocket = new ClientWebSocket())
                {
                    ...
                    dynamic xrplRequest = new ExpandoObject();
                    foreach (var address in addressBatch)
                    {
                        //create request
                    ...
                    addressBatch.ForEach(x => workItems[x] = false);
                    await foreach (var result in ProcessAccountBalanceAsyncEnumerable(clientWebsocket, PeerAccount, LedgerGetBalanceIndex, Currency, workItems, cTokenSource.Token))
                    {
        ...
        private async Task<List<AccountBalance>> ProcessAccountBalanceAsync(ClientWebSocket socket, string peerAccount, uint ledgerGetBalanceIndex, string currency, Dictionary<string, bool> WorkItems, CancellationToken token)
        {
