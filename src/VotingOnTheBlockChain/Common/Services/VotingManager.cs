...
namespace Common.Services
{
    public sealed class VotingManager
    {
        ...
        /// <returns></returns>
        public async IAsyncEnumerable<VotingResults> GetVotingResults(string votingAccount, string votingControllerAccount, uint startIndex, uint endIndex, CancellationTokenSource cTokenSource, string socketEndpoint = "wss://xrplcluster.com/")
        {
                        ...
                        using (var reader = new StreamReader(ms, Encoding.UTF8))
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(responseMessage))
                            {
                                ...
                                if (responseJson != null && successResponse)
                                {
                                    ...
                                    //Process 
                                    if (correlationId == runId.ToString())
                                    {
                                        activeRequestsSend--;
                                        if (jsonResult.RootElement.GetProperty("lines").GetArrayLength() > 0)
                                        {
                                            ...
                                            {
                                                var currency = jsonResult.RootElement.GetProperty("lines")[i].GetProperty("currency").GetString().HexToString();
